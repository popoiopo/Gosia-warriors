...
// var tooltip_format = d3.time.format("%a %d %b");
...
d3.csv("MCBuildingProxSensorData/csv/hazium_data.csv", function(error, data) {
  ...
  data.forEach(function(d){
    ...
    d.F_2_Z_4 = + d.F_2_Z_4;
    d.F_3_Z_1 = + d.F_3_Z_1;
    console.log(d.F_2_Z_2, d.F_1_Z_8A, d.F_2_Z_4, d.F_3_Z_1, d.Time )
  })
  ...
  zone.append("text")
   ...
   // // add circle of crosshair
   // graph.selectAll('circle').data(data).enter().append('circle')
   //              .attr('cx', function(d) { return x(d.date); })
   //              .attr('cy', function(d) { return y(d.Hazium); })
   //              .attr('r', 3)
   //              .attr('class', 'circle');
   ...
   //                  .attr('x1', x(xDomain[0])).attr('y1', y_coor)
   //                  .attr('x2', x(xDomain[1])).attr('y2', y_coor);

   //              // show tooltip	
	  //           tooltip.style("visibility", "visible");		
	  //        	// text in tooltip
	  //           tooltip.html("<b> Date: </b>" + tooltip_format(d.date) + "<br/>"  + "<b> Mean temperature: </b>" + d.mean)	
	  //                  .style("left", (d3.event.pageX) + 50 + "px")		
	  //                  .style("top", y_coor + 100 + "px");   
