...
function changeF2Header() {
    d3.select("#f2-vis-info").text($("#f2-dropdown :selected").text());
    updateMap(eval("f2"+$("#f2-sensors").val()));
...
function initMap() {
	...
	d3.xml("VAST_EnergyZones_F2.svg", "image/svg+xml", function(error, xml) {
	  if (error) throw error;
	  document.body.appendChild(xml.documentElement)

      var data_zones = ["zone1", "zone2", "zone3", "zone4", "zone5", "zone6", "zone7", "zone8", "zone9", "zone10", "zone11", "zone12a", "zone12b", "zone12c", "zone14", "zone15", "zone16"]
	  var svg2_zones = ["#zone1", "#zone2", "#zone3", "#zone4", "#zone5", "#zone6", "#zone7", "#zone8", "#zone9", "#zone10", "#zone11", "#zone12a", "#zone12b", "#zone12c", "#zone14", "#zone15", "#zone16"]
	  var values = []

      for (var i = data_zones.length - 1; i >= 0; i--) {
      	  values.push(f2lightsPower[data_zones[i]][0]["val"]);
	  };    
	  ...
	  var paletteScale = d3.scale.linear()
  	  ...
  	  for (var e = svg2_zones.length - 1; e >= 0; e--) {
  	  	  d3.select(svg2_zones[e]).style("fill", paletteScale(values[e])).style("opacity", 0.6);
      };
	});
};
...
function updateMap(chosen_var) {
    ...
    for (var e = svg2_zones.length - 1; e >= 0; e--) {
        d3.select(svg2_zones[e]).style("fill", paletteScale(chosen_var[data_zones[e]][0]["val"])).style("opacity", 0.6);
    };
}
